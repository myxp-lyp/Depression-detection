...
# Define parameters
parser = argparse.ArgumentParser(description="PHQ Score Prediction with Pre-trained LoRA model")
parser.add_argument("--base_model_path", type=str, default="/data/yl7622/emotion_detection/models--meta-llama--Llama-2-7b-hf")
...
parser.add_argument("--output_dir", type=str, default="qa_transcript_checkpoints")
...
def train():
    ...
    # Apply new LoRA configuration on top of pre-trained model
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                for participant_id in participant_ids:
                    ...
                    all_preds.append(avg_participant_pred)
    ...
    best_model = PeftModel.from_pretrained(pretrained_model, os.path.join(args.output_dir, "best_model"))
    best_model.eval()
...
def inference(model_path, text_input):
    ...
    pretrained_model = PeftModel.from_pretrained(base_model, pretrained_lora_path)
    
    # Load PHQ LoRA model
    model = PeftModel.from_pretrained(pretrained_model, phq_lora_path)
    model.eval()
