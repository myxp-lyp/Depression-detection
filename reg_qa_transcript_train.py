...
parser.add_argument("--base_model_path", type=str, default="models--meta-llama--Llama-2-7b-hf")
parser.add_argument("--pretrained_lora_path", type=str, default="qa_lora_checkpoints/qa_lora_final")
...
parser.add_argument("--output_dir", type=str, default="reg_qa_transcript_checkpoints")
...
def flatten_and_mean(score_lists):
    all_scores = [score for sublist in score_lists for score in sublist]
...
def train():
    ...
    print(f"Loading pre-trained LoRA weights from {args.pretrained_lora_path}...")
    pretrained_model = PeftModel.from_pretrained(base_model, args.pretrained_lora_path)
    
    # Configure additional LoRA layer for PHQ prediction
    print("Configuring additional LoRA parameters for PHQ prediction...")
    ...
    projector = ValueProjector(32000,1).to(device)
    projector = projector
    # Create datasets
    ...
    # Optimizer
    optimizer = torch.optim.Adam(list(projector.parameters()) + list(model.parameters()), lr=1e-4)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in progress_bar:
            ...
            # Clear gradients
            optimizer.zero_grad()
            outputs = model(input_ids=input_ids).logits  # [batch, seq_len, vocab_size]

        # 取最后一层的均值作为回归值
            predictions = projector(outputs[:, -1])#outputs[:, -1, 0]  # [batch]
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                for i in range(len(participant_ids)):
                    ...
                    predictions = projector(outputs[:, -1]).view(-1)#outputs[:, -1, 0]  # [batch]

                    all_preds.append([t.item() for t in predictions.cpu()])
        ...
        avg_pred = pred_df.groupby('Participant_ID')['pred_score'].apply(flatten_and_mean).reset_index(name='pred_score')
    ...
    model = PeftModel.from_pretrained(pretrained_model, os.path.join(args.output_dir, "best_model"))
    model.eval()
    with torch.no_grad():
        all_preds = []
        all_participant_ids = []
        ...
        # Get true PHQ scores for validation set
        true_phq = test_phq_df[['Participant_ID', 'PHQ8_Score']]
        ...
        print(f"Epoch {epoch+1}/{args.epochs} - Train Loss: {avg_train_loss:.4f}")
        print(f"Testing results - RMSE: {rmse:.4f}, MAE: {mae:.4f}, Evaluated participants: {len(merged_df)}")
